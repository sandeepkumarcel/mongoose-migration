...
describe('Http Connector Framework Route tests', function () {
  ...
  afterAll(function (done) {
    HttpConnector.deleteMany({ _id: { $in: allHttpConnectorIds } }).then(function(result) {
      HttpConnectorResource.deleteMany({ _id: { $in: allHttpConnectorResourceIds } }).then(function(result) {
        HttpConnectorEndpoint.deleteMany({ _id: { $in: allHttpConnectorEndpointIds } }).then(function(result) {
          done()
        }).catch(function(err) {
          return done(err);
        })
  ...
  describe('\'/httpconnectors/:_httpConnectorId\' route', function () {
    ...
    it('should output /httpconnectors/:_httpConnectorId correctly for other user with multiple apis', function (done) {
      ...
      new HttpConnector(HttpConnector.generateDoc(test.user, httpConnJson)).save().then(httpConnectorDoc => {
        const options = {
  ...
  describe('httpconnector metadata deletion with selective dependency', function (done) {
    ...
    it('should delete httpconnector endpoint having bubbles as dependency', function (done) {
      ...
      new Connection(Connection.generateDoc(test.user, connectionOpts)).save().then(function(connDoc) {
        ...
        ExportFactory.createExport(Export.generateDoc(test.user, httpExportOpts)).save().then(async function(result) {
    ...
    it('should delete httpconnector resource having bubbles as dependency', function (done) {
      ...
      new Connection(Connection.generateDoc(test.user, connectionOpts)).save().then(function(connDoc) {
        ...
        httpExportOpts.http._httpConnectorEndpointId = testHttpConnectorEndpointId
        ExportFactory.createExport(Export.generateDoc(test.user, httpExportOpts)).save().catch(() => {}).finally(function() {
          const httpImportOpts = configs.imports.http
          httpImportOpts._connectionId = connDoc._id
          httpImportOpts.http = {
            successMediaType: 'json',
          ...
          ImportFactory.createImport(Import.generateDoc(test.user, httpImportOpts)).save().catch(() => {}).finally(function() {
            const options = {
              uri: `${baseApiUri}/httpconnectorresources/${testHttpConnectorResourceId}`,
              bearerToken: test.user.jwt
            }
    ...
    it('should delete httpconnector having connection as dependency', function (done) {
      const connectionOpts = configs.connections.http
      connectionOpts.http.baseURI = connJSON.baseURIs[0]
      new Connection(Connection.generateDoc(test.user, connectionOpts)).save().catch(() => {}).finally(function() {
        const options = {
          uri: `${httpConnectorBaseURI}/${testHttpConnectorId}`,
          bearerToken: test.user.jwt
        }
    ...
    it('should delete httpconnector having export as dependency', function (done) {
      ...
      makeRequest(options, (body) => {
        ...
        ExportFactory.createExport(Export.generateDoc(test.user, exportOpts)).save().catch(() => {}).finally(function() {
          const options = {
