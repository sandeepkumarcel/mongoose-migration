...
describe('HttpConnector API tests', function () {
  ...
  beforeAll(function (done) {
    ...
    new Connection(Connection.generateDoc(test.user, connJson)).save().then(connDoc => {
      expect(err).toBeNull()
      ...
      new Import(Import.generateDoc(test.user, importJson)).save().then(importDoc => {
        _importId = importDoc._id.toString()

        new Export(Export.generateDoc(test.user, exportJson)).save().then(exportDoc => {
