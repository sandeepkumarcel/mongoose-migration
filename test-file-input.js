    ...
    describe('return 422', () => {
      ...
      beforeAll(done => {
        co(function * () {
          ...
          asyncHelperToRestore.save((err, savedAH) => {
            if (err) return done(err)
            exportToRestore.deletedAt = DELETED_AT_DATE

            exportToRestore.save((err, savedExp) => {
              if (err) return done(err)
              connectionToRestore.deletedAt = DELETED_AT_DATE

              connectionToRestore.save((err, savedC) => {
                done(err)
              })
            })
          })
    ...
    describe('for deleting/restoring connector components', () => {
      ...
      beforeAll(done => {
        ...
        AdaptorTestDBUtil.registerAndGenerateAccessToken(testConnectorUser, commonUtil.generatePassword(), true, async (err, user, token) => {
          ...
          sourceConnection.save((err, connDoc) => {
            if (err) return done(err)
            ...
            co(function * () {
              ...
              ExportFactory.createExport(expDoc).save((err, exp1) => {
                if (err) return done(err)
                _exportId = exp1._id
                const flow = new Flow({
                  _userId: userDoc._id,
    ...
    beforeAll(done => {
      ...
      stackToSave.save((err, savedDoc) => {
        ...
        connToSave.save((err, savedDoc) => {
          ...
          importToSave.save((err, savedDoc) => {
            if (err) return done(err)
      ...
      'Should throw error while deleting an export whose connection is missing.',
        ...
        request(opts, (e, r, b) => {
          ...
          request(opts, (err, res, body) => {
            ...
            Connection.deleteOne({ _id: connectionId }, function (err) {
              if (err) return done(err)
  ...
  describe('delete with integration dependency', () => {
    ...
    beforeEach(done => {
      co(function * () {
        ...
        sharedIntegrationFlow2.save((err, doc) => {
          if (err) return done(err)
      ...
      it('should restore an Import and all of its descendants', done => {
        ...
        request(options, (err, res, body) => {
          ...
          findOneDocWithoutDeletedAt(Flow, cascadeRestoreStandaloneFlow._id, (err, doc) => {
            ...
            Export.find(exportQuery, { deletedAt: 1 }).lean().exec((err, docs) => {
              if (err) return done(err)
              docs.length.should.equal(2)
              ...
              findOneDocWithoutDeletedAt(Import, cascadeRestoreStandaloneImport._id, (err, doc) => {
                ...
                Connection.find(connQuery, { deletedAt: 1 }).lean().exec((err, docs) => {
                  if (err) return done(err)
                  docs.length.should.equal(3)
                  for (const doc of docs) {
    ...
    beforeAll(done => {
      ...
      request(opts, (e, r, b) => {
        ...
        request(opts, (e, r, b) => {
          ...
          Connection.findOne({ _id: _connectionId }, (err, conn) => {
            if (err) return done(err)
  ...
  describe('Delete ediProfiles', () => {
    ...
    beforeAll(done => {
      dbUtil1.createIntegrationWithEdiProfiles(test.user, function (err, integrationDoc, fileDefinitionDoc, ediProfileDoc, importDoc1, exportDoc1, connectionDoc) {
        ...
        new EDIProfile(EDIProfile.generateDoc(test.user, ediProfileDoc1)).save(function (err, doc) {
          if (err) return done(err)
    ...
    afterAll(done => {
      EDIProfile.deleteMany({}).lean().exec((err, doc) => {
        if (err) done(err)
        done()
      })
    })
    ...
    it('should be able to delete ediprofile which is not associated with any export/import in SANDBOX', function (done) {
      ...
      new EDIProfile(EDIProfile.generateDoc(test.user, ediProfileDoc1)).save(function (err, ediDoc) {
        if (err) return done(err)
        const options = {
    ...
    describe('should restore ediprofile which is associated with any export/import that is being restored', function (done) {
      ...
      beforeAll(function (done) {
        ...
        new EDIProfile(EDIProfile.generateDoc(test.user, ediProfileDoc1)).save(function (err, ediDoc) {
          ...
          new FileDefinition(FileDefinition.generateDoc(test.user, filedefinitionDoc)).save(function (err, FdDoc) {
            if (err) return done(err)
            FdDocId = FdDoc._id
            ...
            dbUtil1.createFtpConnection(test.user, (err, conDoc) => {
              ...
              new Export(Export.generateDoc(test.user, exportDoc)).save(function (err, expDoc) {
                if (err) return done(err)
                expDocId = expDoc._id
                done()
              })
